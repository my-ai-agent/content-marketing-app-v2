...
function generateCulturalContent(story: string, culturalContext: any): string {
...
Discover ${culturalContext.culturalElements.join(', ')} while respecting the rich cultural heritage of Aotearoa. Our approach honors ${nzCulturalContext.culturalValues.join(', ')}, ensuring every visitor experiences true manaakitanga.

Experience ${culturalContext.seasonalHighlights.join(', ')} with the guidance of those who know this land best. This is more than sightseeing - it's cultural connection with respect and authenticity at its core.

Haere mai - you are welcome here. 🇳🇿`
}

// Enhanced download format content generator - 3 versions per format
function generateDownloadVersions(
  story: string,
...
// ---- ALL GENERATE* FUNCTIONS BELOW NOW HAVE EXPLICIT : string RETURN TYPE ----
...
*Expert Analysis | Industry Leadership | ${new Date().toLocaleDateString('en-NZ')} | ${readingTime} min read*`
    ...
    default:
      // Fix for recursive default: always return 'professional'
...
"${story.split('"')[1] || story.split('.')[1]?.trim()}," said [Spokesperson Name], [Title] at ${company}. "${story.split('"')[3] || 'This represents a significant milestone for our industry and we look forward to continued success.'}"
...
**Why This Matters to You:**
${profile ? `As someone focused on ${profile.valuePropositions?.[0] || 'growth and innovation'}, this development offers new opportunities for ${profile.valuePropositions?.[1] || 'success'}.` : 'This development is relevant to your interests and professional goals.'}
...
P.S. ${profile?.communicationStyle?.includes('casual') ? 'Don\'t forget to share this with your network if you found it valuable!' : 'Please feel free to forward this to colleagues who might find this of interest.'}

[Unsubscribe] | [Update Preferences] | [View in Browser]`
}

function generateStaffNews(
  story: string,
  title: string,
  date: string
): string {
...
function generateStakeholderLetter(
  story: string,
  title: string,
  company: string,
  date: string

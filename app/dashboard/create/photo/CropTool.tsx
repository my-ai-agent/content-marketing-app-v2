...
const CropTool: React.FC<CropToolProps> = ({ image, onApply, onCancel }) => {
  ...
  // FIXED: Convert displayed crop box to natural image pixels (Copilot's solution)
  function getCropPixels() {
    ...
    // CRITICAL FIX: Use actual displayed image size for scale calculation
  ...
  useEffect(() => {
    ...
    if (aspect === "none") {
      return // Don't show crop box for "No Crop"
    }

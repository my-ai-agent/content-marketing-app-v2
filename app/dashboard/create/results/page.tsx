        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0, 0, 0, 0.6)', zIndex: 1001, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'white', borderRadius: '20px', maxWidth: '500px', width: '90%', maxHeight: '80vh', overflow: 'hidden', boxShadow: '0 20px 40px rgba(0, 0, 0, 0.3)' }}>
              ...
              {publishStep === 'preview' && (
                <>
                  <div style={{ marginBottom: '1.5rem' }}>
                    ...
                    <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '1rem', fontSize: '0.9rem', lineHeight: '1.5', color: '#374151', minHeight: '120px' }}>
                      {caption}
                    </div>
                  </div>
                  ...
                  <p style={{ color: '#6b7280', fontSize: '0.9rem' }}>⚠️ This will publish your content to {platformData[currentPlatform as keyof typeof platformData]?.name}. Make sure you're ready before confirming.</p>
                </>
              )}
              {publishStep === 'success' && (
            ...
            <div style={{ background: '#f8fafc', padding: '1.5rem', borderTop: '1px solid #e2e8f0', display: 'flex', gap: '1rem' }}>
              <button onClick={() => setShowPublishModal(false)} style={{ flex: 1, padding: '0.75rem 1rem', background: '#e5e7eb', color: '#374151', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>
                Cancel
              </button>
              <button onClick={handleConfirmPublish} disabled={publishStep === 'setup' && !selectedAccount} style={{ flex: 1, padding: '0.75rem 1rem', background: publishStep === 'setup' && !selectedAccount ? '#d1d5db' : '#3b82f6', color: 'white', border: 'none', borderRadius: '8px', cursor: publishStep === 'setup' && !selectedAccount ? 'not-allowed' : 'pointer', fontWeight: '600' }}>
          ...
          <div style={{ background: 'white', borderRadius: '20px', maxWidth: '600px', width: '90%', maxHeight: '80vh', overflow: 'hidden', boxShadow: '0 20px 40px rgba(0, 0, 0, 0.3)' }}>
            ...
            <div style={{ padding: '1.5rem', maxHeight: '50vh', overflowY: 'auto' }}>
              {versionOptions.map((version, index) => (
                <div key={index} onClick={() => setSelectedVersionIndex(index)} style={{ background: selectedVersionIndex === index ? '#eff6ff' : '#f8fafc', border: selectedVersionIndex === index ? '2px solid #3b82f6' : '2px solid #e2e8f0', borderRadius: '8px', padding: '1rem', marginBottom: '1rem', cursor: 'pointer' }}>
            ...
            <div style={{ background: '#f8fafc', padding: '1.5rem', borderTop: '1px solid #e2e8f0', display: 'flex', gap: '1rem' }}>
              <button onClick={() => setShowRefreshModal(false)} style={{ flex: 1, padding: '0.75rem 1rem', background: '#e5e7eb', color: '#374151', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>
                Cancel
              </button>
              <button onClick={handleSelectVersion} style={{ flex: 1, padding: '0.75rem 1rem', background: '#10b981', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>
                Use This Version
              </button>
            </div>
          </div>

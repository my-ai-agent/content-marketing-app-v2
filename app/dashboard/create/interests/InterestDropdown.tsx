...
export default function InterestDropdown({
  ...
  const handleMobileSelect = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    console.log('Mobile interest selection:', value)
    setSelectedInterest(value)
  }

  // Desktop handler - existing complex logic
  const handleDesktopSelect = (interestValue: string) => {
    console.log('Desktop interest selection:', interestValue)
  ...
  // 📱 MOBILE RENDER: Native HTML Select (Fixes scroll conflicts and modal issues)
  if (isMobile) {
    return (
      <>
        ...
        {isDropdownOpen && (
          <div style={{
            ...
            {interests.map((interest, index) => (
              <div
                key={interest.value}
                onClick={() => handleDesktopSelect(interest.value)}
                onMouseEnter={() => setTooltipInterest(interest)}
                onMouseLeave={() => setTooltipInterest(null)}
                style={{
                  padding: '1rem',
                  cursor: 'pointer',
